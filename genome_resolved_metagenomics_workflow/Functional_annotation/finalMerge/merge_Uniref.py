...
filenames = sorted(glob.glob("*Uniref_summarize.txt"))
print(filenames)
cols = [i.split('.')[0] for i in filenames] 
fin_cols = ['UniRefID'] + cols
...
#dfs[0].join(dfs[1:])
df_final = reduce(lambda left,right: pd.merge(left,right,on=['UniRefID'], how='outer').fillna(0),dfs)
#df_final.fillna(0)
