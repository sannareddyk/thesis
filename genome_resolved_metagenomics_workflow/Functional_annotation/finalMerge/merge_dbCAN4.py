...
filenames = sorted(glob.glob("*CAZy_summarize.txt"))
print(filenames)

cols = [i.split('.')[0] for i in filenames]
fin_cols = ['CAZyID'] + cols
...
#dfs[0].join(dfs[1:])
df_final = reduce(lambda left,right: pd.merge(left,right,on=['CAZyID'], how='outer').fillna(0),dfs)
